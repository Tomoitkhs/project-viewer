...
async function initDb() {
  try {
    ...
    console.log("PostgreSQL unavailable, using file storage:", err.message);
    useDb = false;
    pool = null;
  }
}
...
io.on("connection", socket => {
  ...
  socket.on("join", async name => {
    ...
    if (useDb && pool) {
      try {
        const r = await pool.query("SELECT name,text,image FROM messages ORDER BY id ASC LIMIT 100");
        r.rows.forEach(row => socket.emit("history", row));
      } catch (err) {
        console.log("DB query failed on join, using file fallback:", err.message);
        messages.forEach(row => socket.emit("history", row));
      }
    } else {
      messages.forEach(row => socket.emit("history", row));
    }
  ...
  socket.on("chat", async data => {
    ...
    if (useDb && pool) {
      try {
        ...
        console.log("DB insert failed, using file fallback:", err.message);
        messages.push(msg);
  ...
  socket.on("admin-clear", async () => {
    ...
    if (useDb && pool) {
      try {
        ...
        console.log("DB clear failed, using file fallback:", err.message);
        messages = [];
        saveMessages(messages);
      }
    } else {
